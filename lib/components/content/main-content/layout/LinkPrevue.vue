<template>
    <div>
        <div id="loader-container" v-if="!response && validUrl" :style="{width:cardWidth}">
            <slot name="loading">
                <div class="wrapper loader-container" :style="{width:cardWidth}">
...
<script>
    // ref: https://github.com/nivaldomartinez/link-prevue/blob/master/src/LinkPrevue.vue
    export default {
        name: 'link-prevue',
        props: {
            url: {
        ...
        methods: {
            ...
            getLinkPreview: function() {
                ...
                let now = new Date()
                if( this.cache[this.url] && now - this.cache[this.url].cacheTime <= this.maxCacheTime ){
                    this.response = this.cache[this.url].response
                    this.$emit("previewFetched")
                    return
                }
