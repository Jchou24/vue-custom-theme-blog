...
// source: https://github.com/raphamorim/canvas-experiments

function Inception(d, w, canvas_id){
    let FPS = 1000;
    let TRAIL_PLAN = ["u","r","d","b","r","c"];
    let pointCopy = function(src, dst){
        dst.x = src.x;
        dst.y = src.y;
        dst.z = src.z;
        return dst;
    };
    let Trail = function(pos, t, plan_i){
    ...
    let quadIn = function(t, b, c, d, dst){
        t /= d;
        dst.x = (c.x-b.x)*t*t+b.x;
        dst.y = (c.y-b.y)*t*t+b.y;
        dst.z = (c.z-b.z)*t*t+b.z;
    };
    let perspective = function(point, camera, dst){
    ...
    let canvas = d.getElementById(canvas_id);
